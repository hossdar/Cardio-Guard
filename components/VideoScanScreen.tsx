...
const VideoScanScreen: React.FC = () => {
    ...
    return (
        <div className="h-full bg-black relative flex items-center justify-center overflow-hidden">
            ...
            <div className="relative w-[200px] h-[200px] rounded-2xl overflow-hidden shadow-2xl border border-gray-800">
                {/* Camera Feed */}
                <video
                    ref={videoRef}
                    autoPlay
                ...
                {recordingState === 'idle' && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/40">
                        ...
                        <p className="text-white font-medium mt-2 text-[10px] bg-black/50 px-2 py-0.5 rounded-full">Tap to Record</p>
                    </div>
                )}

                {recordingState === 'recording' && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                        <div className="w-full h-full border-[8px] border-red-500/20 absolute inset-0 animate-pulse"></div>
                        <div className="text-4xl font-bold text-white drop-shadow-lg">{timeLeft}</div>
                    </div>
                )}
                ...
                {recordingState === 'review' && uploadStatus !== 'uploading' && uploadStatus !== 'processing' && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center p-2 bg-black/60">
                        ...
                        <div className="flex gap-2 w-full px-2">
                            <button onClick={retake} className="flex-1 py-1.5 rounded bg-white text-gray-800 text-[10px] font-bold hover:bg-gray-100">
                                Retake
                            </button>
                            <button onClick={confirmUpload} className="flex-1 py-1.5 rounded bg-blue-600 text-white text-[10px] font-bold hover:bg-blue-700">
                                Analyze
                            </button>
                        </div>
                    </div>
                ...
                {(uploadStatus === 'uploading' || uploadStatus === 'processing') && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80 z-20">
                        ...
                        <p className="text-white font-medium text-[10px]">Processing...</p>
                        <p className="text-blue-400 text-[10px] mt-0.5">{progress}%</p>
                    </div>
                )}

                {error && (
                    <div className="absolute bottom-2 left-2 right-2 bg-red-50 text-red-600 p-2 rounded-lg border border-red-100 flex items-center gap-1 text-[10px]">
